...
class RelationModel(DynamicModel):
    ...
    train_fp16: bool = False  # True cause some error
    # train_fp16: bool = True   # The original set
...
class RelationGenerator(RelationModel):
    ...
    def generate_by_head_tail(
        self, head: str, tail: str, rel: str, num: int, pipe: Pipeline
    ) -> Tuple[List[RelationSentence], List[str]]:
        set_seed(self.random_seed)
        encoder = self.get_encoder()
        prompt = encoder.encode_x(head, tail)     # The base is a single parameter, but our HeadTailEncoder use two
        sents, raw = [], []
        errors = set()
        ...
        while len(sents) < num:
            ...
            for o in outputs:
                ...
                x, y = encoder.parse_line(raw[-1])
                try:
                    s = encoder.decode(y=y, head=head, tail=tail, label=rel)
                    if s.is_valid():                
                        sents.append(s)
                except Exception as e:
                    errors.add(str(e))
    ...
    def generate_by_head_tail_with_prefix(
        self, head: str, tail: str, rel: str, num: int, pipe: Pipeline
    ) -> Tuple[List[RelationSentence], List[str]]:
        set_seed(self.random_seed)
        encoder = self.get_encoder()
    ...
    def generate_by_template(
        self, head: str, tail: str, rel: str, template: str, num: int, pipe: Pipeline
    ) -> Tuple[List[RelationSentence], List[str]]:
        """
        try several batches to generate candidate sentences
        """
        set_seed(self.random_seed)
        encoder = self.get_encoder()
        # prompt = encoder.encode_x(head, tail, template)
        prompt = encoder.encode_prompt(head, tail, template)
        sents, raw = [], []
        errors = set()
        ...
        while len(sents) < at_least_num:
            ...
            for output in outputs:
                ...
                x, y = encoder.parse_line(raw[-1])
                try:
                    # We use head, tail, rel to help build a RE instance
                    s = encoder.decode(y=y, head=head, tail=tail, label=rel)
                    if s.is_valid():                        
                        sents.append(s)
                except Exception as e:
...
class HeadTailGenerator(RelationModel):
    """
    head + tail -> sentence
    """
    model_name: str = "gpt2"
    block_size: int = 128
    encoder_name: str = "head_tail_generate"
    pipe_name: str = "text-generation"
    ...
    def generate(
        self, s: str, o: str, num: int, pipe: Pipeline
    ) -> Tuple[List[RelationSentence], List[str]]:
        set_seed(self.random_seed)
        encoder = self.get_encoder()
        prompt = encoder.encode_x(s, o)     # The base is a single parameter, but our HeadTailEncoder use two
        sents, raw = [], []
        errors = set()
